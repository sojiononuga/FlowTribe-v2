...
assert(fastChat.includes('max_tokens: 180'), 'Griot generation budget must stay bounded for responsiveness.');
...
assert(fastChat.includes("Return only the natural-language answer"), 'Fast Griot must avoid structured-output latency and JSON debris.');
...
assert(gateway.includes("OPENAI_SPEECH_VOICE = 'marin'"), 'Fallback Griot voice must use Marin.');
...
assert(index.includes('styles/mobile-repair.css'), 'The mobile repair stylesheet must remain loaded.');
